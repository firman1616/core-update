$(document).ready(function () {
	...
	$("#edit_container_truck")
		.select2({
			...
			ajax: {
				...
				processResults: function (data) {
					// console.log("DEBUG AJAX CONTAINER DATA:", data); // ⬅️ cek hasil AJAX
					return {
		...
		.on("select2:select", function (e) {
			...
			// console.log("DEBUG SELECTED FROM DROPDOWN:", selected); // ⬅️ cek hasil pilih

			$("#edit_max_volume").val(selected.max_volume);
			$("#edit_max_payload").val(selected.max_payload);

			if (typeof refreshProgressBarFromInputs === "function") {
				refreshProgressBarFromInputs();
			}
		});

	// ====== Preselect Data dari DB (edit mode) ======
	// console.log("DEBUG PRESELECT FROM DB:", {
	// 	selectedId,
	// 	selectedText,
	// 	maxVolume,
	// 	maxPayload,
	// }); // ⬅️ cek nilai dari PHP

	if (selectedId && selectedText) {
		let option = new Option(selectedText, selectedId, true, true);
		$("#edit_container_truck").append(option).trigger("change");
		// console.log("DEBUG OPTION APPENDED:", option); // ⬅️ cek option yg ditambahkan

		$("#edit_max_volume").val(maxVolume);
		$("#edit_max_payload").val(maxPayload);
	}
	...
	// Tambah row baru
	$(document).on("click", ".edit-add-row", function () {
		if (!$("#edit_container_truck").val()) {
			Swal.fire({
				icon: "error",
				title: "Oops...",
    ...
    `;
		...
		$(".select-barang")
			...
			.on("select2:select", function (e) {
				...
				row
					.find("input[name='konv[]']")
					.val(isNaN(parseFloat(data.konv)) ? 0 : parseFloat(data.konv));
	...
	$(document).on("click", ".julian-row", function () {
		let row = $(this).closest("tr");
		let detStuffId = row.index(); // pakai index row sebagai key sementara
		let no_so = row.find("select[name='no_so[]']").val();
		...
		// kalau ada data temp sebelumnya, tampilkan
	...
	// let julianTempData = {}; // <--- deklarasi global

	// // === Inisialisasi julianTempData dari data PHP ===
	// if (
	// 	typeof existingJulianData !== "undefined" &&
	// 	Array.isArray(existingJulianData)
	// ) {
	// 	existingJulianData.forEach((detail, index) => {
	// 		if (Array.isArray(detail.julian) && detail.julian.length > 0) {
	// 			julianTempData[detail.id] = detail.julian.map((j) => ({
	// 				julian: j.no_julian,
	// 				line: j.line,
	// 				tahun: j.tahun,
	// 				qty: j.qty_julian,
	...
	// 		}
	// 	});
	// 	console.log("✅ Julian data loaded into julianTempData:", julianTempData);
	// }
	...
	$(document).on("click", "#saveJulianModal", function () {
		...
		Swal.fire({
			icon: "success",
			title: "Data Julian Disimpan Sementara",
	...
	// show data julian code
	$(document).on("click", ".edit-julian-row", function () {
		let row = $(this).closest("tr");
		let detStuffId = row.index(); // tetap pakai index row, bukan langsung simpan DB
		let detId = $(this).data("id");
		...
		$("#julianModal").data("mode", "temp");
		$("#julianModal").data("detStuffId", detStuffId);
		$("#julianModal").data("detId", detId); // tetap simpan detId untuk referensi update nanti

		// Reset modal
		$("#julianModalLabel").text("Edit Data Julian - SO: " + (no_so || "-"));
		$("#julianBarang").html("<p>Nama Barang : " + (barang || "-") + "</p>");
		$("#julianInfo").html("<p>Qty Plan : " + (qty || 0) + "</p>");
		$("#julianTable tbody").empty();

		// Jika sebelumnya user sudah edit & tersimpan di temp, pakai data temp
		if (julianTempData[detStuffId]) {
			julianTempData[detStuffId].forEach((item, i) => {
				let newRow = `
			<tr>
			...
			// kalau belum ada di temp, load dari DB dulu
			$.get(BASE_URL + "Stuffing/get_julian_data/" + detId, function (res) {
	...
	$(document).on("click", "#update-data", function (e) {
		...
		// ==== 1. Validasi Header Form ====
		let buyer = $("#edit_buyer").val();
		let tipeTransportasi = $("#edit_tipe_transportasi").val();
		let containerTruck = $("#edit_container_truck").val();
		let komoditas = $("#edit_komoditas").val();

		if (!buyer || !tipeTransportasi || !containerTruck || !komoditas) {
			Swal.fire({
				icon: "warning",
				title: "Form Belum Lengkap",
				text: "Harap lengkapi semua field header!",
			});
			return;
		}

		// ==== 2. Ambil Data Header ====
		let headerData = $("#editformStuffing").serializeArray();
		let headerObj = {};
		headerData.forEach((item) => (headerObj[item.name] = item.value));

		// ==== 3. Ambil Data Detail + Julian ====
		let details = [];
		$("#table-mc tbody tr").each(function (i, row) {
			let detId = $(row).find(".edit-julian-row").data("id"); // id detail stuffing lama (jika ada)
			let detail = {
				id: detId || null,
				...
				julian: julianTempData[detId] || [], // ambil data julian dari array sementara
			};
			details.push(detail);
		});
		...
		// ==== 5. Kirim AJAX ke Controller ====
		$.ajax({
			url: BASE_URL + "Stuffing/update",
			type: "POST",
...
document.addEventListener("DOMContentLoaded", function () {
	let tglStuff = document.getElementById("edit_tgl_stuff");
...
function updateTotalQty() {
	...
	// buang desimal (pastikan bilangan bulat)
	total = Math.floor(total); // atau Math.round(total)

	// kalau ada perubahan qty, timpa nilai database
	$("#edit_total_qty_plan").val(
		total > 0 ? total : $("#edit_total_qty_plan").val()
	);
}
...
function updateTotalVolume() {
	...
	$("#edit_total_volume").val(total.toFixed(2)); // isi total volume

	// === Update Progress Bar ===
	let maxVolume = parseFloat($("#edit_max_volume").val()) || 0; // ambil dari input hidden/id max_volume
...
function updateTotalBerat() {
	...
	$("#edit_total_weight").val(total.toFixed(3)); // bisa disesuaikan berapa decimal

	// === Update Progress Bar ===
	let masPayload = parseFloat($("#edit_max_payload").val()) || 0; // ambil dari input hidden/id max_volume
